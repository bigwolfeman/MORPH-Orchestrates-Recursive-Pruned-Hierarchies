...
**Date:** 2026-06-19 · **Origin:** the framing during the β1=0 AdEMAMix stability campaign (Task #276).
...
*scrambled* basin ~45–58, not the healthy ~28 → deploy-unacceptable per 's "doesn't return to baseline =
...
3. **The "scrambled recovery" is operator-level FALSE** — E3a's loss recovered to ppl~70 but σ_max stayed ~1e6,
 realized gain ~1e3; RMSNorm `final_norm` masks the wrecked operator in the loss. ⇒ 's "periodic
...
VJP per step) — the rigorous probe, to build (autograd-graph care through the compiled core).
...
- This addresses the bifurcation itself, and — unlike a magnitude clamp — should let us **keep β1=0 (the
 memory win) AND α·m_slow (the training gain won't give up).**
...
**Broader payoff :** this reframes optimizer design for *all* looped/recursive/weight-shared
