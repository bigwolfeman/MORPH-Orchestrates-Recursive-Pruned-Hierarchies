...
## Post-carve regression (Wolfe's report, now quantified)
...
4. **Router cost (regime 4, 70% of run)**: bit-exact reductions only — hoist the topk
   ...
   fp32→bf16 router is class B (numerics). Big structural option (needs Wolfe/ablation:
...
Class B (surface with evidence, Wolfe opt-in): GradScaler removal under bf16 (scale is
