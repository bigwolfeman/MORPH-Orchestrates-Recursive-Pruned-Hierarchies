# Agent Note: Ab Monitoring Resume

Status: implemented

Origin: Ai-notes/06-01-2026/MORPH-AB-Monitoring/resume.md
...
## ⚡ IMMEDIATE TASK: monitor the dense A/B campaign overnight (operator away)
...
- cap ~8h
On exit: read the relevant log tail, diagnose, and react (see failure modes). Then report in the morning.
...
loss starts ~11.6, should fall steadily (flat LR 1e-4). sps: probe showed ~1.36 warmup-incl at B4; steady likely ~2. tokens/sec ~22k at B4. peak_mem_alloc: eager ~30 GiB, kernels ~12.5 GiB (THE headline A/B number needed).
...
Per the planned sequence: optimizer/ternary tests → pruning. End goal: ternary shadow + 8-bit AdamW together.
...
3. **Lorentz dormancy experiments** (flagged: embeddings are FLAT, radius 0.069 ≈ init, geometrically ~Euclidean):
...
- Commit/push ONLY when asks. wandb FULL config (Hydra handles). Don't run 2 models on one GPU.
