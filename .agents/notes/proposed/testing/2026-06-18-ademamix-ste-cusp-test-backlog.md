...
NEXT DISCRIMINATOR (no run pending, locked decision): log cos **per loop-iteration index** (not
...
- ▶ **T1. Pause pruning 17500→19600, same gate+clip** *(short-replay, approved ~30 min)*.
 DECISIVE: if the detonation DISAPPEARS → trigger is the prune schedule. **NOTE : we've seen
...
- ✅ **B1 fp32-state — DETONATED @~18200** (VAL 32.81@18000 → ppl 122@18200 → 50,199@19000), even
 ...
 culprit** (the hypothesis FALSIFIED). Full-precision optimizer state didn't help. `ignore/B1_fp32_state_noprune.*`
...
- ❌ **L1 τ=1.3 — MAGNITUDE GOVERNOR FALSIFIED AS CURE** (2026-06-18): peak ppl **784@18400 ≈ τ=2's
 ...
 flow (hence the recovery-SPEED difference), not the spike. 's "periodic scrambling → won't
 ...
 **(prior fork, now being worked via M1 — NOT shipping AdamW yet):**
 ...
 scramble; per the deploy bar that's not deployable. The β1=0 memory win (~1 B/param) may be unreachable.
- ◽ (was) **L1 core-gain governor τ=2.0 — RUNNING** (PID 3905725, locked decision). BUILT: per-iteration
