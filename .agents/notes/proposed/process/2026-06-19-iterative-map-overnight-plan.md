...
- POS (expected): AdamW survives + σ_max STAYS ~tens → the ARCHITECTURE tolerates σ_max~22; β1=0 NOISE drives
  the runaway → **cure = OPTIMIZER-SIDE σ_max control (preserves β1=0 memory)** is viable + preferred (Wolfe's
...
(measure with E2). DO NOT force σ_max≤1 (lobotomizes). Hold full launch for Wolfe's go on the architecture.
