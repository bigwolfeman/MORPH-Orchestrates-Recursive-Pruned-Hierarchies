...
| 5 | **Router elementwise-tail fusion** | fuse relu/threshold-topk/sum/div-normalize/aux-var per router call (113 calls/step); drop invariant casts | ~-300 to -500 launches (r4 only) | A for the fusion; **B** for bf16 router (Focus-1: numerics + ~0.65 GB) | SPEC (routing.py Triton) | ncu router forward; A-fusion parity trace; bf16 → Wolfe opt-in |
