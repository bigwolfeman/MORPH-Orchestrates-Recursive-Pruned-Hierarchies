...
class CMSBlockLinear(nn.Module):
    ...
    def __init__(
        ...
        # Importance scoring mode for accumulate_scores / prune_step.
        ...
        # Structured masked-dense pruning state (pre-compact). prune_mask is [R, C] bool
        # (True = alive); None until the first prune_step. _prune_elem_mask is the cached
        # [out, in] elementwise mask used to re-zero dead tiles in the live weight every step.
        ...
        # Post-compact ternary QAT on the sparse `values` (Phase C: continue ternary
        # pretraining after compaction). Internal STE flag — NOT torch.parametrize, which
        # cannot bind a tensor named "values" (its parametrizations ModuleDict reserves
        # the .values() method). values stays the smooth shadow; forward applies the STE.
        self._values_ternary_mode = False
        self._values_threshold = 0.5
        # CMS tile-scoring (activation/gradient norm accumulation) is only needed while we
        # still prune. Post-compact (Phase C never prunes again) the hooks are dead compute
        # AND a per-forward memory cost (x*x / grad*grad temps on a model already near the
        # 28 GB usable ceiling once routing activations land). set_scoring(False) at compact.
        ...
        # backend (morph.sparse.stk). Mutually exclusive with the legacy Block-ELL
        # `values` mode — a layer is dense, OR Block-ELL-compacted, OR MORTAR-carved.
        ...
        # Handles are kept so carve() can REMOVE them: post-carve the scoring is
        # permanently dead, and a registered full_backward_hook graph-breaks
        # torch.compile around every CMSBlockLinear call ("Module-level backwards
        # hooks require compiled autograd") — which would keep the carved MLP
        # unfused even after the phase-boundary recompile.
        self._activation_hook_handle = self.register_forward_hook(self._activation_hook)
        self._gradient_hook_handle = self.register_full_backward_hook(self._gradient_hook)
    ...
    def enable_values_ternary(self, threshold: float = 0.5) -> None:
        """Continue ternary QAT on the post-compact sparse ``values`` (per-tensor symmetric).

        ``values`` stays the smooth trainable shadow; the sparse forward applies the STE so
        the effective sparse weight is ternary. Mirrors the dense ``_ternary_mode`` path.
        Per-tensor symmetric (the deploy ``scale_group='tensor'`` config).
        """
        assert not self._dense_mode, "enable_values_ternary requires a compacted (sparse) layer"
    ...
    def set_scoring(self, enabled: bool) -> None:
        """Enable/disable the CMS activation+gradient tile-scoring hooks.

        Scoring is only needed while pruning. Disabling it post-compact (Phase C never prunes
        again) removes the per-forward x*x / grad*grad temporaries — dead compute that also
        costs peak memory once routing activations land. Safe to toggle anytime; the hooks
        remain registered but early-return when disabled.
        """
        self._scoring_enabled = bool(enabled)

    def _effective_values(self) -> Tensor:
        """Sparse weight actually used in forward: ternarized shadow if values-ternary QAT
        is on, else the raw smooth ``values``. Per-tensor symmetric STE (forward = scale·q,
        gradient = identity → grad flows to the smooth ``values`` shadow)."""
        v = self.values
        if not getattr(self, "_values_ternary_mode", False):
            return v
        scale = v.detach().abs().mean().clamp(min=1e-8)
        v_norm = v / scale
        q = torch.sign(v_norm) * (v_norm.abs() > self._values_threshold).to(v.dtype)
        return v + (scale * q - v).detach()
    ...
    def _activation_hook(
        ...
        """
        if not self.training or not self._scoring_enabled:
            return
    ...
    def _forward_reference(self, x: Tensor) -> Tensor:
        ...
        block_outputs = torch.einsum("bsrki,rkoi->bsrko", input_gathered, self._effective_values())
    ...
    def accumulate_scores(self) -> None:
        ...
        if self._dense_mode:
            ...
            v_grad = self.values.grad
            if v_grad is None and parametrize.is_parametrized(self, "values"):
                v_grad = self.parametrizations.values.original.grad
            if v_grad is None:
                return
            if self.score_mode == "grad":
                grad_norms = compute_gradient_frobenius_norms(v_grad)
            elif self.score_mode == "taylor":
                grad_norms = compute_gradient_frobenius_norms(self.values.detach() * v_grad)
            elif self.score_mode == "magnitude":
                grad_norms = compute_gradient_frobenius_norms(self.values.detach())
            else:
                raise ValueError(f"unknown score_mode {self.score_mode!r}")
    ...
    # ── Structured masked-dense pruning (pre-compact) ───────────────────────
    # Drives real density reduction by ZEROING the lowest-saliency tiles in the
    # dense weight (the swap-only topology_step keeps density fixed at K). Kept
    # masked (not compacted) so there is no parameter swap / optimizer rebuild:
    # the network function at 25% density is identical to its compacted twin.
    ...
    def prune_step(self, prune_rate: float, target_density: float) -> dict:
        """Zero the lowest-saliency alive tiles, per row (row-uniform → exact density).

        Saliency = block_score_ema (whatever score_mode produced it: grad / taylor / magnitude).
        Returns {"pruned": n_tiles_dropped, "density": new_density}.
        """
        if not self._dense_mode:
            return {"pruned": 0, "density": self.prune_density()}
        self._ensure_prune_mask()
        cur_alive = int(self._prune_mask[0].sum().item())  # row-uniform invariant
        target_K = max(1, int(round(target_density * self.C)))
        if cur_alive <= target_K:
            return {"pruned": 0, "density": cur_alive / self.C}
        new_alive = int(math.floor(cur_alive * (1.0 - prune_rate)))
        new_alive = max(target_K, min(new_alive, cur_alive - 1))  # guarantee progress
        n_drop = cur_alive - new_alive
        with torch.no_grad():
            saliency = self.block_score_ema.detach().clone().float()  # [R, C]
            # dead tiles → +inf so they are never re-selected as "lowest"
            saliency = saliency.masked_fill(~self._prune_mask, float("inf"))
            _, drop_idx = saliency.topk(n_drop, dim=1, largest=False)  # [R, n_drop]
            self._prune_mask.scatter_(1, drop_idx, False)
        self._rebuild_prune_elem_mask()
        self.apply_prune_mask()
        return {"pruned": int(n_drop * self.R), "density": new_alive / self.C}
    ...
    def compact(self) -> int:
        ...
        with torch.no_grad():
            ...
            if self._dense_mode:
                ...
                # compact() predates parametrize-based ternary QAT: under
                # torch.nn.utils.parametrize the leaf "weight" is moved out of _parameters into
                # parametrizations.weight.original, so a bare del _parameters["weight"] KeyErrors.
                # Remove the parametrization first (leave_parametrized=True keeps the ternary-
                # EFFECTIVE values, matching w_data captured above) — then the del succeeds.
                import torch.nn.utils.parametrize as _parametrize
                if _parametrize.is_parametrized(self, "weight"):
                    _parametrize.remove_parametrizations(self, "weight", leave_parametrized=True)
                ...
                # Same parametrize-awareness as the dense branch (values may carry a
                # post-compact ternary parametrization).
                import torch.nn.utils.parametrize as _parametrize
                if _parametrize.is_parametrized(self, "values"):
                    _parametrize.remove_parametrizations(self, "values", leave_parametrized=True)
    ...
    def _init_mortar_storage(
        ...
        self._fp8_mode = False
        self.disable_topology_updates = True
        self.set_scoring(False)
        self.invalidate_column_index_cache()
        # Scoring/topology is over forever — remove the (now-dead) capture hooks.
        # Not cosmetic: a registered full_backward_hook forces a Dynamo graph
        # break around every call of this module, un-fusing the carved MLP.
        for _h in (getattr(self, "_activation_hook_handle", None),
                   getattr(self, "_gradient_hook_handle", None)):
            if _h is not None:
                _h.remove()
        self._activation_hook_handle = None
        self._gradient_hook_handle = None
    ...
    def carve(self, blocking: int = 128) -> int:
        ...
        The MORTAR successor to compact(). Where compact() re-packs into the 16×16
        Block-ELL `values` format (whose kernel measured 4.5× slower than cuBLAS at
        equal FLOPs — below MMA granularity), carve() packs into hardware-grain
        128×128 BCSR blocks executed by the vendored stk Triton backend (measured
        3.09× FASTER than dense at 0.25 density, Gate G1).
        ...
        Ternary deploy stack: like the schedule's compact path, callers restore the
        smooth shadow (remove_parametrizations leave_parametrized=False) before
        calling, and re-enable per-tensor STE on the carved data after via
        enable_values_ternary() — QAT continues on mortar_data uninterrupted.

        Returns nnz (number of kept blocks).
        """
        assert self._dense_mode, "carve() requires dense mode (pre-compact)"
    ...
    def _mortar_effective_data(self) -> Tensor:
        ...
        mortar_data when values-ternary QAT is on (same formula as _effective_values —
        forward = scale·q, gradient = identity into the smooth mortar_data shadow)."""
        d = self.mortar_data
        if not getattr(self, "_values_ternary_mode", False):
            return d
    ...
    def _load_from_state_dict(self, state_dict, prefix, *args, **kwargs):
        ...
        nn.Module.load_state_dict recurses through THIS hook (not the public
        load_state_dict override above), so a freshly-built dense model loads a
        post-carve checkpoint without external surgery: detect mortar keys, install
        empty storage with the checkpoint's shapes, let super() fill it.
        """
