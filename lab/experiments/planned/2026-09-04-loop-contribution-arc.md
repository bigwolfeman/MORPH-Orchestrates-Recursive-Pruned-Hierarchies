...
| per-pass strength | Can a ternary core be a strong per-pass map? Four one-factor arms on the Parcae-entry recipe: `scale-norm-match` (new scale mode: ternary norm = latent norm), `scale-ttq` (learnable scales), `threshold-0.3` (narrower dead zone), `precision-bf16-all` (the ceiling). Read on the K-curve and the core MLP branch out/in per pass | PLANNED `planned/2026-09-09-arc-per-pass-strength.md` (frozen 2026-09-09; chained behind the horizon run). Behind it: the measured 0.668 norm shrink and 30.5 % dead zone on the Parcae-entry core; the prior-art search found no ternary looped model. |
