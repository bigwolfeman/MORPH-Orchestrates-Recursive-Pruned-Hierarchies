...
| E19 | Parcae's loop entry on the PLAIN model (Wolfe 2026-09-09: "We misimplemented the looping in MORPH. It has never been contributing."): state from noise, e re-injected on every dim through a learned B, no fixed-point term; arms parcae-entry / plain-no-fixed-point / plain-depth1 | **FAILURE** `failures/2026-09-09-arc-e19-parcae-loop-entry.md` (filed 2026-09-09). The entry is NOT the lobotomy: parcae-entry K1-K6 0.033, K3-K6 0.0009, flat to T16, yet 0.023 nats BETTER than the plain arm token-paired and start-independent (4 inits reach 3.896 by T4); B/decay/dt untrained. HEADLINE: plain@6 - depth1@1 = +0.019 [+0.017, +0.021] (the loop was FREE in training; the depth-1 model trains in 0.33 h vs 0.88-1.00 h); parcae-entry@6 - depth1@1 = -0.004. The fixed-point term alone: CE unchanged, K1-K6 0.037. |
| E20 | Three loop-depth candidates on the Parcae-entry arm, one factor each: `depthcand-dense-core` (ternary everywhere but the core), `depthcand-carry-lr20x` (injection params at 20x LR), `depthcand-draw8-bptt4` (Parcae's schedule) | **FAILURE** `failures/2026-09-09-arc-e20-loop-depth-candidates.md` (filed 2026-09-09). The schedule is NOT the lever (0.092 worse, K3-K8 0.002, loses to depth-1 by 0.088); a trained carry is NOT the lever (B 1.74, carrier rank 6, 0.037 worse). The ternary core IS a depth limiter: bf16 core -> K1-K6 0.033 -> 0.168, K3-K6 0.0009 -> 0.012, 0.030 better than the ternary base and 0.034 better than depth-1, MLP branch out/in 0.2 -> 0.9 per pass. Next: core precision UNDER ternary (Agent Note), then the density panel's reading. |
