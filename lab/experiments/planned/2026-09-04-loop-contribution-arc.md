...
| E19 | Parcae's loop entry on the PLAIN model (Wolfe 2026-09-09: "We misimplemented the looping in MORPH. It has never been contributing."): state from noise, e re-injected on every dim through a learned B, no fixed-point term; arms parcae-entry / plain-no-fixed-point / plain-depth1 | **FAILURE** `failures/2026-09-09-arc-e19-parcae-loop-entry.md` (filed 2026-09-09). The entry is NOT the lobotomy: parcae-entry K1-K6 0.033, K3-K6 0.0009, flat to T16, yet 0.023 nats BETTER than the plain arm token-paired and start-independent (4 inits reach 3.896 by T4); B/decay/dt untrained. Depth-1 control at this horizon: plain@6 - depth1@1 = +0.019 [+0.017, +0.021], parcae-entry@6 - depth1@1 = -0.004 (a 5,000-step reading, not a ranking of looped vs unlooped: deep models converge slower). The fixed-point term alone: CE unchanged, K1-K6 0.037. |
| E20 | Three loop-depth candidates on the Parcae-entry arm, one factor each: `depthcand-dense-core` (ternary everywhere but the core), `depthcand-carry-lr20x` (injection params at 20x LR), `depthcand-draw8-bptt4` (Parcae's schedule) | **FAILURE** `failures/2026-09-09-arc-e20-loop-depth-candidates.md` (filed 2026-09-09). The schedule does not move loop contribution (K3-K8 0.002; 0.092 behind at 5k); a trained carry does not either (B 1.74, carrier rank 6, K3-K6 0.007). The ternary core limits loop contribution: bf16 core -> K1-K6 0.033 -> 0.168, K3-K6 0.0009 -> 0.012, MLP branch out/in 0.2 -> 0.9 per pass. CE gaps at 5k are not verdicts on looping or ternary (Wolfe). Next: core precision UNDER ternary (Agent Note), then the density panel's reading. |
| density | Block-prune side result (ran on a misreading of "test density", which meant the weight data type): `density-half` / `density-quarter` on the Parcae-entry recipe, prune only | **FAILURE** `failures/2026-09-09-arc-density-panel.md` (filed 2026-09-09). Not a loop-contribution lever: quarter K1-K6 0.074 (dense 0.033) with a WEAKER per-pass map (branch out/in 0.058 vs 0.236) and a rank-4 carrier, K3-K6 0.005; half K1-K6 0.043, K3-K6 0.003. CE gaps at 5k are horizon readings. |
| precision | The weight-precision axis on loop contribution (Wolfe 2026-09-09: "we needed to test ternary vs bf16"): ternary everywhere (parcae-entry, K1-K6 0.033) and bf16 core only (depthcand-dense-core, 0.168) already ran; `precision-bf16-all` (no ternary anywhere) is the third point | PLANNED `planned/2026-09-09-arc-precision-axis.md` (frozen 2026-09-09; its queued arm was KILLED before starting on Wolfe's order 10:35, the horizon run takes the GPU first). |
| horizon | Does the ternary loop's contribution change over training? The surviving 20k plain control (old entry) swept at 8 checkpoints; the Parcae-entry recipe run to 10k | **FAILURE** `failures/2026-09-09-arc-horizon-ternary-25k.md` (stopped at 10.2k on Wolfe's decision, 2026-09-09). FLAT: old entry K1-K6 0.036-0.042 and K3-K6 0.001-0.003 from 2,500 to 20,000 while CE falls 4.45 -> 3.45; core MLP branch out/in creeps 0.17 -> 0.26; new entry 0.035/0.038 at 5k/10k. Training length is not the lever. |
| per-pass strength | Can a ternary core be a strong per-pass map? Four one-factor arms on the Parcae-entry recipe: `scale-norm-match` (new scale mode: ternary norm = latent norm), `scale-ttq` (learnable scales), `threshold-03` (narrower dead zone), `precision-bf16-all` (the ceiling). Read on the K-curve and the core MLP branch out/in per pass | **SUCCESS** `successes/2026-09-09-arc-per-pass-strength.md`. The scale rule was the limiter: norm-match K1−K6 0.185, K3−K6 0.014, core MLP branch 0.86–1.10 (above the bf16-core diagnostic on every instrument); the dead-zone arm reads as the base (0.026 / 0.0004); the learnable scales SHRINK to 0.87× absmean and read below the base (0.041 / 0.002, branch 0.08–0.14). CE at 5k +0.08 vs the base (a horizon reading). Binding applied the same day: `base.yaml` `ternary_scale_mode: norm_match`, the rule carried through the carve and the packer (`morph/model/ternary_rule.py`). `precision-bf16-all` deferred to the next GPU window. Open: the paid TUL loop under norm_match, the 20k horizon. |
