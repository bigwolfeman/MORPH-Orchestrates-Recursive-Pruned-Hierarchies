...
\node[sub, text=cbOrange, left=2.4cm of ca, align=right]
  (csid) {mHC residual\\$\gamma_i$ per channel\\$x_0$, bigram};
...
% ── STRIP MAC + LM HEAD ────────────────────────────────────────────
\node[Bs, fill=fGray!30, above=0.8cm of dm] (str) {Strip MAC Tokens};
\draw[A] (dm) -- (str);
...
\node[Bs, fill=fGray!30, above=0.3cm of mix] (fn) {Final RMSNorm};
\draw[A] (mix) -- (fn);

\node[B, fill=fYellow, above=0.3cm of fn] (hd) {Weight-Tied LM Head};
\draw[A] (fn) -- (hd);
