...
\node[sub, text=bdr, anchor=north west, align=left] at ($(HC.north west)+(0.3,-0.72)$) {%
  residual mechanism (deploy default)\\
...
\node[tiny, text=cbOrange!70!black, anchor=south west, align=left]
  at ($(HC.south west)+(0.3,0.15)$) {%
  alt: \texttt{residual\_mode=null} $\Rightarrow$ MRR\\(per-channel $\gamma_i$ multi-rate residual)};
...
\node[sub, text=bdr!70, below=0.35cm of leg, anchor=north west, align=left,
      text width=12.5cm] {%
