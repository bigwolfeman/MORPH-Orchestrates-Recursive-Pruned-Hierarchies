...
%
% NOTE (No-Theater): deploy default (base.yaml) is embed_quant="off" == bit-identical bf16.
% The int6 coloring below is the DEPLOYMENT TARGET (Ablation E validated), matching the
% morph_deploy_stack.tex precision legend (int6 embeddings / bf16 Lorentz).
...
\node[draw=bdr, rounded corners=6pt, fill=fGray!25, inner sep=10pt,
  ...
  \;Default \texttt{embed\_quant="off"} $=$ bit-identical bf16; int6 shown is the \emph{deployment target}.};
