...
\tikzset{
  ...
  gs/.style={draw=bdr, rounded corners=3pt, minimum width=2.5cm,
    minimum height=0.72cm, font=\scriptsize\sffamily, align=center, inner sep=3pt},
...
\begin{scope}[on background layer]
  \node[draw=cbBlue!45, thick, rounded corners=7pt, fill=cbBlue!4,
        fit=(s1)(s7), inner xsep=7pt, inner ysep=10pt] (STRIP) {};
\end{scope}
% Label sits fully above the strip (white fill clears the border). Keep the
% public name — internal class renames must not lengthen this caption.
\node[font=\small\sffamily\bfseries, text=cbBlue, anchor=south west,
      fill=white, inner sep=2pt]
  at ([xshift=0.15cm, yshift=0.10cm] STRIP.north west)
  {CCA prologue \;{\footnotesize(shared channel-compress $C{=}2$ $\cdot$ $d_h{=}d/(C H)$)}};
...
\node[Bx, fill=fOrange, draw=cbOrange!85!black, line width=1pt,
      minimum width=3.9cm, inner sep=6pt, anchor=south] (win) at (-3.2,3.4) {
  \textbf{Window branch}\;{\footnotesize(LOCAL)}\\[3pt]
  {\scriptsize \textbf{per-token} attention over}\\[1pt]
...
\node[draw=cbGray, densely dashed, very thick, rounded corners=7pt, fill=cbGray!4,
      minimum width=7.9cm, minimum height=4.55cm, anchor=south] (slot) at (3.75,3.0) {};
\node[tny, text=cbGray, anchor=south east, fill=white, inner sep=1.5pt]
  at ([xshift=-0.18cm, yshift=0.10cm] slot.north east)
...
\node[Bx, fill=cbBlue!8, draw=cbBlue, line width=1pt, minimum width=8.4cm,
      inner sep=7pt, anchor=south] (comb) at (0.4,7.9) {
  \textbf{Gate blend $+$ residual-$\alpha$ $+$ sinks}\\[3pt]
  {\scriptsize per-head sigmoid gate MLP $d\to d/4\to 2H$ (2 wts/head): $g_c\,o_\mathrm{comp}+g_w\,o_\mathrm{win}$}\\[2pt]
