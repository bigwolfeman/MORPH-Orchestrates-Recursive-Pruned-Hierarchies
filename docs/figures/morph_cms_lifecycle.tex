% MORPH — Core-MLP Lifecycle: MORTAR · CMS pruning · ReMoE routing.
% Companion to morph_block.tex (the static forward view). Tells the TRAINING-LIFECYCLE story
% via the SwiGLU-MLP weight as a block-grid evolving across phases.
%
% Verified against code (2026-06-11, post Block-ELL rip):
%   - CMSBlockLinear dual-mode: dense [out,in] (cuBLAS F.linear) -> carve() -> MORTAR BCSR
%     mortar_data[nnz,128,128] + row_indices/column_indices/offsets (vendored morph/sparse/stk).
%   - prune_step_blocks: score at 16x16 tile (block_score_ema, score_mode=taylor/grad/magnitude),
%     execute at 128x128 block — zero the lowest-saliency blocks toward target_density.
%   - carve() packs the kept 128x128 blocks into BCSR (stk Triton fwd+bwd; 3.09x FASTER than
%     dense at 0.25 density, Gate G1). MORTAR is the ONLY sparse backend (no scope knob).
%   - Applied to prelude + core + coda SwiGLU MLPs (gate_up + down).
...
\begin{scope}[xshift=4.4cm]
  ...
  \node[badge=cbOrange] at (\bcx,0.40) {block prune};
  ...
  \node[dsc, text width=3.5cm] at (\bcx,-2.35)
    {keep highest-saliency $128\!\times\!128$ blocks; zero the rest};
  \node[nte, text=bdr, text width=3.5cm] at (\bcx,-3.55)
    {\texttt{prune\_step\_blocks} toward \texttt{target\_density} $=0.25$};
\end{scope}
...
\begin{scope}[xshift=8.8cm]
  \draw[draw=cbBlue, very thick, rounded corners=7pt, fill=cbBlue!4]
    (\bxl,\bxt) rectangle (\bxr,\bxb);
  \node[ttl, text=cbBlue] at (\bcx,0.82) {B2 · Carve};
  \node[badge=cbBlue] at (\bcx,0.40) {dense $\to$ MORTAR BCSR};
  ...
  \node[dsc, text width=3.5cm] at (\bcx,-2.35)
    {\texttt{mortar\_data}$[\mathrm{nnz},128,128]$\\ $+$ BCSR \texttt{row}/\texttt{col}/\texttt{offsets}};
  \node[nte, text=bdr, text width=3.5cm] at (\bcx,-3.55)
    {stk Triton fwd $+$ bwd; $3.09\times$ faster than dense @\,0.25};
\end{scope}
...
\node[draw=bdr, rounded corners=6pt, fill=fGray!25, inner sep=11pt,
  ...
  \;$\cdot$\; \texttt{carve()} once at \texttt{compact\_step} \;$\cdot$\; ReMoE at \texttt{route\_start}.
  \;Saliency $=$ \texttt{score\_mode} (taylor / grad / magnitude) EMA, pooled $16\!\times\!16$ tile $\to$ $128\!\times\!128$ block (score at tile, execute at block).\\[4pt]
  \textbf{ReMoE:} routing is \emph{per-token activation-conditioned} on each MORTAR layer (prelude / core / coda).
...
\node[font=\Large\sffamily\bfseries, anchor=south] at (8.4,4.25)
  {MORPH --- Core-MLP Lifecycle: MORTAR $\cdot$ CMS Pruning $\cdot$ ReMoE};
