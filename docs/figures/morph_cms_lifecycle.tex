% MORPH — MLP Lifecycle: MORTAR · CMS pruning · ReMoE routing.
...
%   - carve() packs the kept 128x128 blocks into BCSR for stk Triton fwd+bwd.
%     MORTAR is the ONLY sparse backend (no scope knob).
...
\begin{scope}[xshift=8.8cm]
  ...
  \node[nte, text=bdr, text width=3.5cm] at (\bcx,-3.55)
    {stk Triton fwd $+$ bwd for carved sparse blocks};
\end{scope}
...
\node[font=\Large\sffamily\bfseries, anchor=south] at (8.4,4.25)
  {MORPH --- MLP Lifecycle: MORTAR $\cdot$ CMS Pruning $\cdot$ ReMoE};
