...
% ── Precision palette: Okabe-Ito (colorblind-safe; bf16/ternary maximally distinct) ──
\definecolor{pBf16}{HTML}{0072B2}   % blue
\definecolor{pTern}{HTML}{D55E00}   % vermillion (orange-red)
\definecolor{pInt6}{HTML}{009E73}   % bluish green
\definecolor{pInt4}{HTML}{CC79A7}   % reddish purple
...
\node[L, fill=fTeal, above=0.5cm of inp] (emb) {
  Hybrid Embedding\\[-1pt]
  {\scriptsize Euclidean + Bigram (quant) $\;\oplus\;$ Lorentz (bf16)}};
\draw[A] (inp) -- (emb);
\precb{emb}{pInt6}{int6\\(per-row)}

\node[L, fill=fBlue!55, above=0.6cm of emb] (pre) {
  \textbf{Prelude} $\times N$\\[-1pt]
  {\scriptsize attn + SwiGLU; layer 1 $\to$ +GLA}};
\draw[A] (emb) -- (pre);

% ── CORE LOOP (the heart) ────────────────────────────────────────────
\node[L, fill=fBlue!35, above=0.7cm of pre, minimum height=1.5cm,
      draw=loopC, line width=2pt] (core) {
  \textbf{Core Loop} $\times T \sim \mathrm{Poi}(\lambda{=}6)$\\[-1pt]
  {\scriptsize attn + SwiGLU\,(Block-ELL); layer 1 $\to$ +GLA}\\[-1pt]
  {\scriptsize truncated BPTT (last 4 iters)}};
\draw[A] (pre) -- (core);

\node[L, fill=fBlue!55, above=0.7cm of core] (coda) {
  \textbf{Coda} $\times N$\\[-1pt]
  {\scriptsize attn + SwiGLU; layer 1 $\to$ +GLA}};
\draw[A] (core) -- (coda);

\node[Ls, fill=fGray, above=0.4cm of coda] (mix) {LM-Head Mixer + RMSNorm};
\draw[A] (coda) -- (mix);

\node[L, fill=fYellow, above=0.4cm of mix] (head) {
  Weight-Tied LM Head\\[-1pt]{\scriptsize fused linear cross-entropy (fp32 logits)}};
\draw[A] (mix) -- (head);
\precb{head}{pBf16}{bf16\\(tied)}
...
\node[anchor=west, font=\scriptsize\sffamily, align=left, text=bdr,
      inner sep=2pt] (bb) at ($(core.east)+(0.85,0)$) {%
  backbone\\[1pt]\textcolor{pTern}{\textbf{MLP ternary}}\\\textcolor{pBf16}{\textbf{attn bf16}}};

% =====================================================================
% LEFT PANEL — architecture carriers (cross-cutting, all bf16)
% =====================================================================

\node[panel=loopC, anchor=east, text width=4.7cm] at ($(core.west)+(-1.3,1.4)$) (hc) {
  \textbf{\textcolor{loopC}{Cayley Hyper-Connections}}\\[2pt]
  ...
  {\tiny \textbf{bf16} — replaces MRR as deploy default}};

\node[panel=cbPurple, anchor=east, text width=4.7cm, below=0.5cm of hc] (gla) {
  \textbf{\textcolor{cbPurple}{Retention (GLA) branch}} \,{\tiny layer 1 of prelude / core / coda}\\[2pt]
  ...
  {\tiny core: $S_T$ carried across loop iters (ablation \#230)}\\
...
\draw[Ad, loopC] (hc.east) -- ($(core.west)+(-0.4,1.2)$);
\draw[Ad, cbPurple] (gla.east) -- ($(core.west)+(-0.4,-1.2)$);

% =====================================================================
% RIGHT PANEL — train-time / inference-time overlays
% =====================================================================

\node[panel=p8bit, anchor=west, text width=4.5cm] at ($(core.east)+(3.6,0.55)$) (opt) {
  \textbf{\textcolor{p8bit}{Optimizer (training)}}\\[2pt]
  8-bit AdamW (bitsandbytes)\\
  {\tiny state $m,v \to$ uint8; embed state forced fp32}\\
  {\tiny composes free w/ ternary ($\Delta$ppl $-0.36$)}};

\node[panel=pInt4, anchor=west, text width=4.5cm, below=0.5cm of opt] (kv) {
  \textbf{\textcolor{pInt4!60!black}{KV cache (inference)}}\\[2pt]
  int4 PTQ on the growing CSA term\\
  {\tiny $3.5\times$ memory @128k ctx; $\Delta$ppl $\approx -0.20$}\\
  {\tiny separate decode path — not in train forward}};

% both overlays connect to the backbone brace (a real target, not empty space)
\draw[Ad, p8bit] (opt.west) -- ([yshift=4pt]bb.east);
\draw[Ad, pInt4] (kv.west) -- ([yshift=-4pt]bb.east);
...
\node[draw=bdr, rounded corners=6pt, fill=fGray!25, inner sep=9pt,
  ...
  \tikz\node[badge=pInt4, inner sep=2pt]{int4};~KV cache \;\;
  \tikz\node[badge=p8bit, inner sep=2pt]{8-bit};~optimizer state\\[5pt]
  \textbf{Validated evidence} (15k, b4, vs bf16 baseline \textbf{ppl 39.46}):\;
  ternary backbone $\to$ 42.07 ($+6.6\%$ for $22.9\%$ of params $\to\{-1,0,1\}$);\;
  int6 embeds $\approx$ free ($72\%$ of params);\;
  attn left bf16 (all-ternary attn $=44.92 \gg$);\;
  int4 KV $\Delta$ppl $\approx 0$;\;
  8-bit AdamW interaction $-0.36$.};
