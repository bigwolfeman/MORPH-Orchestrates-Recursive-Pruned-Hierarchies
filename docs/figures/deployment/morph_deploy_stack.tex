...
% training-time optimizer overlay (8-bit AdamW).
...
% normalization / carriers, and 8-bit AdamW optimizer state during training.
...
\node[PT, above=0.4cm of coda, minimum height=0.7cm] (mix) {LM-Head Mixer\\[-1pt]{\scriptsize mix ternary; norm bf16}};
\draw[A] (coda) -- (mix);

\node[L, above=0.4cm of mix] (head) {
  Weight-Tied LM Head\\[-1pt]{\scriptsize Euclidean side int6; Lorentz side bf16; fp32 logits}};
\draw[A] (mix) -- (head);
