...
% training-time optimizer overlay (8-bit AdEMAMix).
...
% Brace rails sit outside the shared stack width (all tiers are 6.0cm).
% Amplitude stays inside the gap to the side panels / labels.
\coordinate (braceR) at ($(core.east)+(0.55,0)$);
\coordinate (braceL) at ($(core.west)+(-0.55,0)$);

% ── backbone precision: ONE brace spanning prelude->coda (all 3 sections identical) ──
\draw[decorate, decoration={brace, amplitude=8pt}, thick, bdr]
  ($(coda.north -| braceR)+(0,0.12)$) -- ($(pre.south -| braceR)+(0,-0.12)$);
\node[anchor=west, font=\scriptsize\sffamily, align=center, text=bdr,
      inner sep=2pt] (bb) at ($(braceR)+(0.45,0)$) {%
  backbone\\[1pt]{\tiny MLP/mix ternary}\\[1pt]{\tiny attention bf16}};

% =====================================================================
% LEFT PANEL — architecture carriers (cross-cutting, all bf16)
% =====================================================================

\node[panel=bdr, anchor=east, text width=4.7cm] at ($(braceL)+(-1.55,1.5)$) (hc) {
...
\node[panel=bdr, anchor=east, text width=4.7cm, below=0.5cm of hc] (gla) {
  \textbf{\textcolor{bdr}{Retention (GLA) branch}}\\[1pt]
  {\tiny layer 1 of prelude / core / coda}\\[2pt]
...
\draw[decorate, decoration={brace, amplitude=8pt, mirror}, thick, bdr]
  ($(coda.north -| braceL)+(0,0.12)$) -- ($(pre.south -| braceL)+(0,-0.12)$);
\node[font=\tiny\sffamily, text=bdr, rotate=90, anchor=south]
  at ($(braceL)+(-0.38,0)$) {prelude / core / coda};
% arrows land on the brace rail, not through the panel borders
\draw[Ad, bdr] (hc.east) -- ($(braceL)+(0,1.2)$);
\draw[Ad, bdr] (gla.east) -- ($(braceL)+(0,-1.2)$);

% =====================================================================
% RIGHT PANEL — train-time / inference-time overlays
% =====================================================================

\node[panel=bdr, anchor=west, text width=4.5cm] at ($(bb.east)+(1.1,0)$) (opt) {
...
\node[draw=bdr, rounded corners=6pt, fill=fGray!25, inner sep=9pt,
  ...
  \tikz\node[badge=pTern, inner sep=2pt]{ternary};~MLP $+$ mixer linears \;\;
