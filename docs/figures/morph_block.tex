...
\node[sub, text=cbOrange!70!black, right=0.9cm of yA, align=left]
  {$y=\mathrm{attn}(\bar{x})+\sigma(g)\!\cdot\!\mathrm{GLA}(\bar{x},S)$};
...
\end{scope}
\node[font=\small\sffamily\bfseries, text=cbBlue, anchor=west]
  at ($(AG.north west)+(0.15,0)$) {Attention sublayer};

% ── MID CARRIER ──────────────────────────────────────────────────────
\node[carrierbar, below=0.85cm of AG] (cmid) {HC carrier $[B,S,4,C]$};
...
\end{scope}
\node[font=\small\sffamily\bfseries, text=cbOrange!80!black, anchor=west]
  at ($(MG.north west)+(0.15,0)$) {MLP sublayer};

% ── OUT CARRIER ──────────────────────────────────────────────────────
\node[carrierbar, below=0.85cm of MG] (cout) {HC carrier $[B,S,4,C]$};
...
\node[draw=cbOrange!70!black, thick, rounded corners=6pt, fill=cbOrange!6,
      anchor=north west, text width=6.1cm, inner sep=8pt,
      font=\scriptsize\sffamily, align=left, below=0.45cm of key] (alt) {%
  \textbf{Alternative residual} (\texttt{residual\_mode=null}): \textbf{MRR} ---
  split $C$ into compute/context/memory and apply per-channel learned gains $\gamma_i$
  ($\gamma\!\approx\!1/0.5/0.1$). Single-stream $[B,S,C]$; ablation arm, not deploy default.};

\node[draw=cbPurple!70!black, thick, rounded corners=6pt, fill=fPurple,
      anchor=north west, text width=6.1cm, inner sep=8pt,
      font=\scriptsize\sffamily, align=left, below=0.45cm of alt] (glan) {%
...
\node[draw=bdr!60, thick, rounded corners=6pt, fill=bdr!4,
      anchor=north west, text width=6.1cm, inner sep=8pt,
      font=\scriptsize\sffamily, align=left, below=0.45cm of glan] (slice) {%
  \textbf{Channel slices} (compute 384 / context 256 / memory 128) persist under HC as
  \emph{injection \& diagonal-decay targets} within each stream's $C$ --- not the residual
  mechanism. The single-stream injection term broadcasts into all $n$ streams.};
