% MORPH — MORPH Block Detail (Diagram 2, v3)
% Shows the 3-channel multi-rate residual stream through one block.
...
% ── Title ────────────────────────────────────────────────────────────
\node[font=\Large\sffamily\bfseries] (title) {MORPH Block};
...
\node[B, fill=fGray!50, below=0.5cm of attn, minimum height=0.85cm] (mhc_a) {
  MRR: \ $h_i = h_i + \gamma_i \cdot o_i$ \quad per channel
};
\draw[A] (attn) -- (mhc_a);
