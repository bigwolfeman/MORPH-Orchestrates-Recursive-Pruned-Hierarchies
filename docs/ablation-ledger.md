...
| Ternary-norm-match | Accept the norm-matching ternary scale (`‖W‖_F/√nnz`, symmetric codes) over absmean | `ternary_scale_mode: norm_match` | The absmean rule shrinks each ternary layer to ~0.67 of its latent norm and starves the looped core (MLP branch out/in 0.19–0.24, K1−K6 0.033); norm_match restores 0.86–1.10 and 0.185 at 5k on the plain Parcae-entry model. Learnable scales (ttq) settle at 0.86× absmean: the optimizer does not grow them. CE at 5k is 0.08 higher (a horizon reading). Carried through the carve and the packer (`morph/model/ternary_rule.py`). Unmeasured: the paid TUL loop and the 20k horizon. | medium | `lab/experiments/successes/2026-09-09-arc-per-pass-strength.md`, `.agents/notes/implemented/architecture/2026-09-09-ternary-core-is-a-weak-per-pass-map.md` |
