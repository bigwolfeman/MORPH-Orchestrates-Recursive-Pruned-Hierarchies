...
- **MORPH uses:** The geodesic smoothness constraint applied to hidden-state trajectories during pretraining - confining intermediate states to lie within a tubular neighborhood of the geodesic connecting segment boundaries on the semantic manifold. MORPH applies STP during pretraining (not fine-tuning as in the paper) with a multi-scale scheme (strides 1,2,4,...,tau=64).
