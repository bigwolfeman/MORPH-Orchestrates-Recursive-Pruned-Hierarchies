...
- **MORPH uses:** The Memory-Augmented Context (MAC) variant: gradient-based surprise update where a deep MLP memory M is updated on the forward pass via momentum-accelerated associative loss minimization - S_t = eta_t*S_{t-1} - theta_t*grad||M(k)-v||^2, M_t = (1-alpha_t)*M_{t-1} + S_t - with alpha/eta/theta as sigmoid(Linear(chunk_pooled)) learned gates, L2-normalized K/Q, and SiLU activation.
